...
def create_event(title: str, start: datetime, end: datetime, description: str = "") -> str:
    service = _get_service()
    fmt = "%Y-%m-%dT%H:%M:%S"
    event = {
        "summary": title,
        "start": {"dateTime": start.strftime(fmt), "timeZone": "Asia/Seoul"},
        "end": {"dateTime": end.strftime(fmt), "timeZone": "Asia/Seoul"},
    }
    if description:
